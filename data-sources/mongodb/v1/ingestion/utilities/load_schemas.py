...
# ——— CONFIGURATION ———
REGISTRY_URL = "http://192.168.0.3:5544/apis/registry/v2"
...
def main():
    ...
    for db_name in os.listdir(SCHEMA_DIR):
        db_path = os.path.join(SCHEMA_DIR, db_name)
        ...
        for fname in os.listdir(db_path):
            ...
            artifact_id = f"{db_name}_{os.path.splitext(fname)[0]}"
            file_path = os.path.join(db_path, fname)

            try:
                # Step 1: Upload RAW schema (unmodified)
                raw_schema = read_schema_from_file(file_path)
                create_or_update_schema(RAW_GROUP_ID, artifact_id, raw_schema)
                time.sleep(1)

                # Step 2: Upload STAGING schema (with metadata)
                staging_schema = inject_metadata_fields(raw_schema)
                create_or_update_schema(STAGING_GROUP_ID, artifact_id, staging_schema)
                time.sleep(1)
